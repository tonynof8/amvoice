      ...
      if (service === 'voice_video') {
        tooltipText.innerHTML = `
          <div style="text-align: center; padding: 20px;">
            ...
            <div style="font-size: 1.2rem; font-weight: 700;">Озвучка видео</div>
            <div style="font-size: 0.95rem; opacity: 0.8;">Длительность округляется в большую сторону.</div>
        ...
        tooltipText.innerHTML = `
          <div style="text-align: center; padding: 20px;">
            <div class="calc-tooltip-icon">🌍🎬</div>
            <div style="font-size: 1.2rem; font-weight: 700;">Перевод + озвучка видео</div>
            <div style="font-size: 0.95rem; opacity: 0.8;">Длительность округляется в большую сторону.</div>
            <div style="font-size: 0.9rem; opacity: 0.7; margin-top: 8px;">Например: 6 минут 3 секунды → 7 минут</div>
          </div>
        `;
...
function calculate() {
  ...
  setTimeout(() => {
    try {
      ...
      `;
      resultText += `
          <div class="calc-order-button-wrap">
                <button type="button" class="btn order" onclick="showContactOptions()">
            Заказать озвучку
        </button>
      `;
...
// ============================================
// СБРОС РЕЗУЛЬТАТА — возврат к окну ввода
// ============================================
window.calcResetResult = function() {
  const fileZone      = document.getElementById('calc-fileZone');
  const textZone      = document.getElementById('calc-textZone');
  const resultContent = document.getElementById('calc-resultContent');
  const textarea      = document.getElementById('calc-textArea');
  ...
  // Сбросить сохранённый результат
  window.lastCalculationResult = '';

  // Скрыть результат
  if (resultContent) resultContent.classList.add('calc-hidden');

  // Показать соответствующую зону ввода (в зависимости от текущего режима)
  if (isTextMode) {
    if (textZone) textZone.classList.remove('calc-hidden');
    if (fileZone) fileZone.classList.add('calc-hidden');
    ...
    // Сброс drag-zone в начальное состояние
    const fileZoneContent = fileZone?.querySelector('.calc-file-zone-content');
    const fileZoneLoader  = fileZone?.querySelector('.calc-file-zone-loader');
    if (fileZoneContent) fileZoneContent.classList.remove('calc-hidden');
    if (fileZoneLoader)  fileZoneLoader.classList.add('calc-hidden');
  }
};
...
// ============================================
window.calcPasteFromClipboard = async function() {
  const textarea = document.getElementById('calc-textArea');
  if (!textarea) return;
  ...
  try {
    ...
    textarea.focus();
    showNotification('⚠️ Нажмите Ctrl+V для вставки');
  }
};
